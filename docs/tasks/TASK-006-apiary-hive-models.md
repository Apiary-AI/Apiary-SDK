# TASK-006: Apiary & Hive Models + HasUlid Trait

**Status:** done
**Branch:** `task/006-apiary-hive-models`
**PR:** [#6](https://github.com/Apiary-AI/Apiary-SaaS/pull/6)
**Depends on:** TASK-002, TASK-005
**Blocks:** TASK-007, TASK-008, TASK-009, TASK-010, TASK-031

## Objective

Create the Apiary and Hive Eloquent models and a reusable HasUlid trait so that
all future models have consistent ULID primary key handling. These are the
foundational models in the Apiary hierarchy — every other core model depends on
...
- [x] FR-3: `Apiary` model maps to `apiaries` table with correct fillable/casts
- [x] FR-4: `Apiary` model has `hives()` HasMany relationship
- [x] FR-5: `Apiary` model has `owner()` BelongsTo relationship to User
...
- [x] FR-11: Cloud-only datetime fields properly cast on Apiary model
...
| Create | `app/Models/Apiary.php` | Apiary (organization) Eloquent model |
...
- **HasUlid wraps Laravel's HasUlids**: Provides a project-level abstraction so ULID behavior is consistent across all Apiary models and can be extended later if needed.
- **Apiary does NOT use BelongsToApiary**: The Apiary model is the root of the hierarchy — it doesn't belong to itself. Only child resources use the scoping trait.
...
2. Create `app/Models/Apiary.php` with HasUlid, fillable, casts, relationships
...
6. Write feature tests for Apiary and Hive model behavior
...
- [x] Apiary can be created with auto-generated ULID
- [x] Apiary does not use auto-increment
- [x] Apiary fillable fields work correctly
- [x] Apiary settings column casts to/from array
- [x] Apiary settings defaults to empty when not provided
- [x] Apiary trial_ends_at casts to datetime
- [x] Apiary has hives() relationship returning HasMany
- [x] Apiary hives() returns child hives
- [x] Apiary has owner() relationship returning BelongsTo
- [x] Apiary owner() returns User
- [x] Apiary cascade deletes child hives
...
- [x] Hive BelongsToApiary trait auto-sets apiary_id in CE mode
- [x] Hive apiary() relationship returns parent Apiary
