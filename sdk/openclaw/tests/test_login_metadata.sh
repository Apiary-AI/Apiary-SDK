#!/usr/bin/env bash
# test_login_metadata.sh — Tests that apiary_oc_login persists agent metadata.
#
...
_setup() {
    unset APIARY_AGENT_ID APIARY_HIVE_ID APIARY_AGENT_NAME APIARY_TOKEN 2>/dev/null || true
    export APIARY_CONFIG_DIR="$_tmp_config_dir"
    export APIARY_BASE_URL="http://localhost:9999"
...
_setup
export APIARY_AGENT_ID="agent-login-01"
export APIARY_AGENT_SECRET="test-secret"
...
set +e
apiary_oc_login > "$_tmp_out" 2>&1
rc=$?
set -e

assert_eq "$rc" "0" "login returns 0"
assert_eq "${APIARY_HIVE_ID:-}" "hive-77" "APIARY_HIVE_ID exported after login"
assert_eq "${APIARY_AGENT_NAME:-}" "my-openclaw" "APIARY_AGENT_NAME exported after login"
...
_setup
export APIARY_AGENT_ID="agent-login-02"
export APIARY_AGENT_SECRET="test-secret"
...
describe "Login preserves env APIARY_HIVE_ID when response has no hive_id"

_setup
export APIARY_AGENT_ID="agent-login-03"
export APIARY_AGENT_SECRET="test-secret"
...
assert_eq "$rc" "0" "login returns 0"
assert_eq "${APIARY_HIVE_ID:-}" "env-hive-99" "preserves existing APIARY_HIVE_ID when response has no hive_id"
assert_eq "${APIARY_AGENT_NAME:-}" "agent-three" "APIARY_AGENT_NAME set from response"
...
_setup
export APIARY_AGENT_ID="agent-login-05"
export APIARY_AGENT_SECRET="test-secret"
# No APIARY_HIVE_ID in env, but write one to agent.json (simulates prior session)
...
assert_eq "$rc" "0" "login returns 0"
assert_eq "${APIARY_HIVE_ID:-}" "persisted-hive-88" "APIARY_HIVE_ID retained from prior agent.json"
assert_eq "${APIARY_AGENT_NAME:-}" "new-name" "APIARY_AGENT_NAME updated from response"
...
_setup
export APIARY_AGENT_ID="agent-login-04"
export APIARY_AGENT_SECRET="test-secret"
# APIARY_HIVE_ID intentionally not set
...
assert_eq "$rc" "0" "login returns 0"
assert_eq "${APIARY_HIVE_ID:-}" "hive-new-42" "APIARY_HIVE_ID populated from login response"
assert_eq "${APIARY_AGENT_NAME:-}" "agent-four" "APIARY_AGENT_NAME populated from login response"
