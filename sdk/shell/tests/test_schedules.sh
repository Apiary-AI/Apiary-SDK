...
describe "apiary_list_schedules"
...
result=$(apiary_list_schedules "$HIVE")
...
apiary_list_schedules "$HIVE" -s "active" >/dev/null
url=$(mock_last_url)
assert_contains "$url" "status=active" "list_schedules sends status filter"

# ── Get schedule ────────────────────────────────────────────────

describe "apiary_get_schedule"
...
result=$(apiary_get_schedule "$HIVE" "$SCHED")
...
describe "apiary_create_schedule"
...
result=$(apiary_create_schedule "$HIVE" -n "nightly-report" -g "cron" -t "generate_report" -c "0 2 * * *" -p 3)
...
assert_exit 1 apiary_create_schedule "$HIVE" -n "test" -g "cron" "create_schedule fails without task_type"

# ── Update schedule ─────────────────────────────────────────────

describe "apiary_update_schedule"
...
result=$(apiary_update_schedule "$HIVE" "$SCHED" -c "0 3 * * *" -p 5)
...
apiary_update_schedule "$HIVE" "$SCHED" -n "renamed" >/dev/null
body=$(mock_last_body)
...
apiary_update_schedule "$HIVE" "$SCHED" -o "skip" -e "2026-12-31T23:59:59Z" >/dev/null
body=$(mock_last_body)
...
describe "apiary_delete_schedule"

mock_reset
mock_response DELETE "/api/v1/hives/${HIVE}/schedules/${SCHED}" 204 ""

apiary_delete_schedule "$HIVE" "$SCHED"
rc=$?
